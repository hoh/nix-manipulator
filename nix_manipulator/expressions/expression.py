...
class NixExpression(BaseModel):
    ...
    def add_trivia(self, rebuild_string: str, indent: int, inline: bool,
                   after_str: Optional[str] = None) -> str:
        from nix_manipulator.format import _format_trivia
        before_str = _format_trivia(self.before, indent=indent)
        after_str = after_str if after_str is not None else _format_trivia(self.after, indent=indent)
        indentation = " " * indent if not inline else ""

        if self.after and self.after[-1] != linebreak and after_str[-1] == "\n":
            after_str = after_str[:-1]

        return f"{before_str}{indentation}{rebuild_string}" + (f"\n{after_str}" if after_str else "")
